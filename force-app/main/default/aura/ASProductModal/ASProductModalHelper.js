({
    getQuoteValue : function(component, event, helper){
        ...
        action.setCallback(this, function(response){
            ...
            if(state === "SUCCESS" && responseData.success !== false){
                component.set("v.targetPriceValue", responseData.content.targetPriceValue);
                component.set("v.renewalFeeValue", responseData.content.renewalFee);
                component.set("v.oldTargetPriceValue", responseData.content.targetPriceValue);
                component.set("v.previewedSubtotal", responseData.content.previewedSubtotal);
                component.set("v.subscriptionType", responseData.content.subscriptionType);
    ...
    insertRequest : function(component, event, helper) {
        ...
        if((oneTimeRequest !== undefined && oneTimeRequest.length > 0)
                            ...
                            if(isApprovalFlow || isGenerateContractFlow){
                                if(metricsChanged){
                                    ...
                                    const errorHandler = function (message) {
                                        console.error('AS MODAL Received error ', JSON.stringify(message));
                                      };
                                    const channelCallback = function(message){
                                        console.log('AS MODAL Event Received : ' + JSON.stringify(message));
                                        helper.onReceiveNotification(component, helper, message);
                                    }
                                    empApi.onError($A.getCallback(errorHandler));
                                    empApi.subscribe(channel, replayId, $A.getCallback(channelCallback)).then($A.getCallback(function (newSubscription) {
                                        console.log('AS modal Subscribed to channel ' + channel);
                                        component.set('v.quoteIsChangedSubscription', newSubscription);
                                    }));
                                } else {
            ...
            helper.showToast('error','Error!','Populate at least one of the request lookups');
        }
