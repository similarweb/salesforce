    ...
    addWordListPicklistValue : function(component,helper,fullNameListRaw) {
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                var responseObj = response.getReturnValue();
                if(responseObj.success){
                    helper.showToast('success','Success!','Word picklist values successfully assigned to \''+ controllingFieldValue + '\' base product!');
    ...
    deleteWordListPicklistValue : function(component,helper,rowName) {
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                var responseObj = response.getReturnValue();
                if(responseObj.success){
                    helper.showToast('success','Success!','\'' +rowName +'\' succesfully deleted!');
                    $A.get('e.force:refreshView').fire();    
                } else {
                    helper.showToast('error','Error!',responseObj.message);
                }
    ...
    editWordListPicklistValue : function(component,helper,newLabel,fullName) {
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                var responseObj = response.getReturnValue();
                if(responseObj.success){
                    helper.showToast('success','Success!','Label is changed to \''+newLabel+'\' succesfully!');
                    $A.get('e.force:refreshView').fire();    
                } else {
                    helper.showToast('error','Error!',responseObj.message);
                }
