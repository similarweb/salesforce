({
  generatePreviewContract: function (component, helper, isSendToZuora) {
    ...
    action.setParams({
      ...
      isValidate: component.get("v.isValidate")
    });
    action.setCallback(this, function (response) {
      ...
      if (state === "SUCCESS") {
        ...
        if (responseObj.success) {
          ...
          helper.checkIfContractGeneratedAndSaveInSf(
            component,
            helper,
            date1,
            isSendToZuora
  ...
  checkIfContractGeneratedAndSaveInSf: function (
    component,
    helper,
    date1,
    isSendToZuora
    ...
    action.setCallback(this, function (response) {
      ...
      if (state === "SUCCESS") {
        ...
        if (responseObj.success) {
          ...
          if (
            buttonLabel == "Proforma Invoice" ||
            buttonLabel == "Generate Proforma"
          ) {
              ...
              [
                {
                  title: "Next",
                  event: {
                    name: "e.lightning:openFiles",
                    body: { recordIds: [responseObj.message] }
                  }
                }
              ]
            );
