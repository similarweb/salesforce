({
	onInit : function(component, event, helper) {
        setTimeout(function(){
            var parentElem;
            var cardlist = document.getElementsByClassName("forceRelatedListSingleContainer"); 
            ...
            if(!parentElem)
                return;
            var parentHeader = parentElem.querySelector("div[class='slds-card__header slds-grid']");            
            
            if(!parentHeader)
                return;
            
            var div = document.querySelector("div[data-component-id='NewRequestOverrideButtonWrapper']");
            ...
            if(parentHeader.childNodes && parentHeader.childNodes.length > 1){
                ...
                var replaceChild = parentHeader.replaceChild(div, parentHeader.lastChild);
    ...
    createRecord : function(component, event, helper) {
        ...
        if(!!recordTypeId){
            if(recordTypeId == '012b0000000UddJAAS' &&
               oppType != 'Renewal' && oppType != 'Upsell' && oppType != 'New Sale'){ //2021-02-11: added New Sale - Itzik Winograd
                helper.setError(component,'A Solution Consulting request may be created for New Sale, Renewal and Upsell opportunities types only');
            }
            else if(recordTypeId == '0120O000000BvvsQAC' &&
                    oppType != 'New Sale' && oppType != 'Upsell'){
                helper.setError(component,'A Solution Engineering request may be created for New Sale and Upsell opportunities types only');
            }
                else {
