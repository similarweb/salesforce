({
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                if (resp.success) {
                    ...
                    var accIssues = cmp.get('v.fieldDetails.Opportunity.Section_in_PRO_w_accuracy_issues__c');
                    accIssues.push({'value':'Desktop visits & engagement','label':'Desktop visits & engagement'});
                    accIssues.push({'value':'Desktop traffic channels','label':'Desktop traffic channels'});
                    accIssues.push({'value':'Mobile web visits & engagement','label':'Mobile web visits & engagement'});
                    accIssues.push({'value':'Mobile web traffic channels','label':'Mobile web traffic channels'});
                    accIssues.push({'value':'Android app metrics','label':'Android app metrics'});
                    accIssues.push({'value':'iOS app metrics','label':'iOS app metrics'});
                    accIssues.push({'value':'Search keywords','label':'Search keywords'});
                    ...
                    accIssues.push({'value':'DI and conversion','label':'DI and conversion'});
                    accIssues.push({'value':'Other','label':'Other'});
                    cmp.set('v.fieldDetails.Opportunity.Section_in_PRO_w_accuracy_issues__c',accIssues);
                	cmp.set('v.opportunity', opp); 
                    
                    if(opp.Competitor_Product__c && opp.Competitor_Product__c.split(';').indexOf('Other') != -1)
                    ...
                    cmp.set('v.fieldDetails.Opportunity.Closed_Lost_Reason__c', relevantReasons);                    
                    var solutionFit = resp.content['solutionFit'] ? resp.content['solutionFit'] : [];
                    if (opp.Use_Cases__c){
                        solutionFit.push(...opp.Use_Cases__c.split(';'));
                    }
                	cmp.set('v.selectedSolutionFit', solutionFit); 
                    if (opp.IsClosed || (opp.Type != 'Renewal' && opp.Type != 'New Sale' && opp.Type != 'Upsell')){
                        cmp.set('v.canBeShown', false);
                    }
                    if (opp.Type == 'Renewal' && opp.Original_Renewal_Date__c){
                        cmp.set('v.formattedRenewalDate', helper.formatDate(opp.Original_Renewal_Date__c));
                    }
    ...
    handleClosedLostChange : function (cmp, event, helper) {
        ...
        if (!isChecked && newVal && selectedClosedReasons && selectedClosedReasons.indexOf(newVal) != -1){
            selectedClosedReasons.splice(selectedClosedReasons.indexOf(newVal), 1);
            cmp.set('v.selectedClosedReasons', selectedClosedReasons);
        } else {
            if (selectedClosedReasons && selectedClosedReasons.length == 2){
                event.getSource().set('v.value', false);
                helper.showToast(cmp.get('v.isLEX'),'warning', undefined, 'You have selected more than two closed lost reasons. Please select up to two reasons and try again');
            } else {
                selectedClosedReasons.push(newVal);
                cmp.set('v.selectedClosedReasons', selectedClosedReasons);
            }
    ...
    submit : function (cmp, event, helper) {
        ...
        if (cmp.get('v.newCycle'))
        	...
        	var solutionFit = cmp.get('v.selectedSolutionFit') ? cmp.get('v.selectedSolutionFit').join(';') : "";
            ...
            action.setCallback(this, function(response) {
                ...
                if (state === "SUCCESS") {
                    var resp = response.getReturnValue();
                    if (resp.success) {
                        helper.showToast(cmp.get('v.isLEX'), 'success', undefined, 'Opportunity closed successfully');
                        /*if (cmp.get('v.isLEX')){
                        ...
                        helper.showToast(cmp.get('v.isLEX'), 'error', undefined, 'Error happened : ' + resp.message);
                    }
    ...
    cancelFlow : function (cmp, event, helper){
        ...
        if (isLEX){
            ...
            navEvt.fire();*/
            window.location = '/lightning/r/Opportunity/' + cmp.get('v.recordId') + '/view';
        } else {
            window.location = '/' + cmp.get('v.recordId');
        }
    }, 
    ...
    selectCompetitorProduct : function (cmp, event){
        ...
        if ( cmp.get('v.selectedCopetitorProducts') &&  cmp.get('v.selectedCopetitorProducts').length == 3){
            event.target.selected = false;
            alert("You have selected more than three competitor products. Please select up to three products and try again.");
            return;
        }
    ...
    addCompetitorProduct : function (cmp, event){
        ...
        for (var i = 0; i < allValues.length; i++){
            if (allValues[i].value != selectedVal)
                updatedAll.push(allValues[i]);
            else 
                selectedObj = allValues[i];
        }
