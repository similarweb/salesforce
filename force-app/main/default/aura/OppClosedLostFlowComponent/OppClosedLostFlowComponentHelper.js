({
    ...
    var fieldDesc = [
      { Contact: ["Department__c"] },
      {
        Opportunity: [
          ...
          "Issues_with_Data_We_Show__c",
          "Missing_coverage_or_feature__c",
          "Missing_Data_or_Feature__c",
          "Deal_Lost_To_a_Competitor__c",
          "Opp_Will_be_Relevant__c",
          "Cycle__c",
          "Section_in_PRO_w_accuracy_issues__c",
          "Primary_Solution__c",
          "Secondary_Solution__c",
          "Secondary_Use_Case__c",
          "Primary_Use_Case__c",
          "CSA_Satisfaction_Score__c"
        ]
      },
      ...
      if (state === "SUCCESS") {
        ...
        if (resp.success) {
          ...
          if (resp.content["Account"].SW_Industry__c)
            cmp.set(
              "v.newAccSWIndustry",
              resp.content["Account"].SW_Industry__c[0]
            );
        ...
        for (var i = 0; i < selectedCompetitors.length; i++) {
          if (selectedCompetitors[i].value == "Other" && !otherCompetitor) {
            ...
            otherComp.setCustomValidity(
              "Please provide details on other compitetors."
            );
            break;
          }
        }
      }
      if (competitorValid && otherComp) otherComp.setCustomValidity("");
    ...
    for (var i = 0; i < lghtInputsToValidate.length; i++) {
      ...
      if (elem) {
        if (typeof elem.reportValidity === "function") {
          if (!elem.reportValidity()) oppIsValid = false;
    ...
    action.setCallback(this, function (response) {
      ...
      if (state === "SUCCESS") {
        ...
        this.showToast(
          ...
          "error",
          undefined,
          "Error happened : " + response.getError()
        );
      }
    });
  ...
  setCheckedReasons: function (cmp, oppReasons) {
    ...
    for (var i = 0; i < allReasons.length; i++) {
      ...
      if (existing.indexOf(value) != -1) allReasons[i].selected = true;
    }
  ...
  setCheckedCompetitorProds: function (cmp, oppExistingOpts) {
    ...
    for (var i = 0; i < allValues.length; i++) {
      ...
      if (!matched) updatedAll.push(allValues[i]);
    }
  ...
  setDefault: function (cmp, opp) {
    ...
    if (!opp.Issues_with_Data_We_Show__c) {
      var allIssues = cmp.get(
        "v.fieldDetails.Opportunity.Issues_with_Data_We_Show__c"
      );
      if (allIssues)
        cmp.set(
          "v.opportunity.Issues_with_Data_We_Show__c",
          allIssues[0].value
        );
    }
    ...
    if (!opp.Competition__c) {
      var allCompetitions = cmp.get(
        "v.fieldDetails.Opportunity.Competition__c"
      );
      if (allCompetitions) cmp.set("v.opportunity.Competition__c", ""); //allCompetitions[0].value
    }
    if (!opp.Deal_Lost_To_a_Competitor__c) {
      var allDealLosts = cmp.get(
        "v.fieldDetails.Opportunity.Deal_Lost_To_a_Competitor__c"
      );
      if (allDealLosts)
        cmp.set(
          "v.opportunity.Deal_Lost_To_a_Competitor__c",
          allDealLosts[0].value
        );
    }
    if (!opp.Cycle__c) {
      var allCycles = cmp.get("v.fieldDetails.Opportunity.Cycle__c");
      if (allCycles) cmp.set("v.newCycle", allCycles[0].value);
    }
  }
});
