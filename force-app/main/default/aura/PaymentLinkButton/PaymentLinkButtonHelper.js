({
    showPaymentLink : function(component) {
        let modalLabel = component.get('v.label') === 'Feature acceptance link' ? 'Link' : 'Payment link';
        let paymentLinkText = component.get('v.paymentLink');
        let paymentLink;
        if(paymentLinkText.indexOf("<a href=") == -1){
            paymentLink = "<a href=" + paymentLinkText + ">" + paymentLinkText + "</a>";
        } else {
    ...
    showRenewalLink : function(component, event) {
        ...
        var message = 'Please click on the button below “Add Recipients” in order to send an email to the customer with the renewal link.<br>';
        ...
        if (component.get('v.additionalRecipientAddress')) {
            ...
            if(emailAddressList.length > 0){
                message += '<br>Recipient(s) - ' + component.get('v.additionalRecipientAddress');
            }
        }
    ...
    clickSendEmail : function(component, helper) {
        ...
        if(emailAddressList == undefined || emailAddressList.length<1){
            helper.showToast('error', 'Error!',  $A.get("$Label.c.QuoteRenewalEmailMissingRecipientsMsg"));
            return;
        }
        this.setRenewalTimestamp(component);
        helper.showModalPopupComment(component, helper, 'Additional text to be sent to the customer', 'Send Email', 'Comments');
    },

    showModalPopupComment : function(component,helper,title,submitLabel,inputLabel) {
        var modalWindowData = {};
    ...
    clickAddRecipient : function(component, helper) {
        helper.showModalPopupComment(component, helper, 'Additional recipients', 'Add Recipients', 'Email');
    },
