({
	...
	reassignSDR : function(cmp, event, helper) {
		helper.generalResponse(cmp,'assignSDR',{'accountId' : helper.getRecordId(cmp)},function(cmp,helper,result,state,params){
			if ( typeof(result) != "undefined") {
	        	if ( result.success ) {
	        		window.location.reload();
				} else {
					console.log('auraResponse : createError Reassign failed '+result.message);
					helper.createError(cmp,'cmd_acknowledged',result.title,result.message);
				}
			}
