({
            ...
            if (state =="SUCCESS") {
	            ...
	            if ( typeof(result) != "undefined") {
	            	...
	            	if ( result.success ) {
						...
						//q=q+'0';
						console.log('auraResponse : createError Reassign failed '+result.message);
						this.createError(cmp,'cmd_acknowledged',result.title,result.message);
					}
	...
	generalResponse : function(cmp,cmdName,params,successFunction,failFunction){
		var action = cmp.get("c."+cmdName);
        ...
        console.log('auraResponse : action return');
            ...
            var state = a.getState();
            if (state =="SUCCESS") {
	            if (typeof(successFunction) == 'function' && successFunction != null ) successFunction(cmp,helperx,result,state,params);
            }
            else if (state === "ERROR") {
                if (typeof(failFunction) == 'function' && failFunction != null ) failFunction(cmp,helperx,result,state,params);
            }
        	this.toggleSpinner(cmp);
        });
        $A.enqueueAction(action);
	...
	deleteComponent : function(cmp,aura_id) {
		...
		console.log(aura_id+' defined?: '+(typeof(dynamicComponentsByAuraId[aura_id]) != "undefined"));
	...
	openModal : function(cmp) {
		this.generalResponse(cmp,'openModalServerSide',{'oppId' : this.getRecordId(cmp)},function(cmp,helper,result,state,params){
			if ( typeof(result) != "undefined") {
	        	if ( result.success ) {
		...
		$A.createComponents([
		    ...
		    function(components, status, errorMessage){
		        ...
		        if (status === "SUCCESS") {
		            ...
		            console.log('auraResponse : message set body, outputText '+JSON.stringify(outputText));
		            message.set("v.body", '');
		            var cmpbdy = cmp.get("v.body");
		            console.log('auraResponse : cmpbdy push message '+JSON.stringify(message));
		            cmpbdy.push(message);
	...
	calculateNewSubQuoteFlowMode : function(cmp,subscriptionType,documentType){
        if (subscriptionType && documentType && subscriptionType.toLowerCase() == 'new subscription') {
	...
	calculateAmendRenewalFlowMode : function(cmp,quoteBusinessType){
        if (quoteBusinessType && quoteBusinessType.toLowerCase().includes('renewal')) {
			...
			action.setCallback(this, function(a) {
				...
				if (state === "SUCCESS") {
					...
					if (resultValue.success) {
						...
						cmp.set('v.isMsaSentToZuoraEnabled', result.isMsaSentToZuoraEnabled);
						...
						cmp.set('v.isDeltaMrrOrSubGreaterThanZero', result.isDeltaMrrOrSubGreaterThanZero);
						cmp.set('v.isCustomerRequestsToPayImmediately', result.isCustomerRequestsToPayImmediately);
						cmp.set('v.isRenewalByClientConfirmation', result.isRenewalByClientConfirmation);
	...
	recordIsUpdated: function(component, result, helper) {
		...
		if(
			((result.zqu__Previewed_Delta_MRR__c > 0.01 || result.zqu__Previewed_Delta_MRR__c  < -0.01) && result.zqu__Previewed_Delta_MRR__c != null) ||
			((result.zqu__Previewed_Delta_MRR__c > 0.01 || result.zqu__Previewed_Delta_MRR__c  < -0.01) && result.zqu__Previewed_SubTotal__c != null)
		){
		...
		if(linkToSWSystem != undefined && linkToSWSystem.length > 0){
			if(linkToSWSystem.includes("_HL_")){
				...
				linkToSWSystem = '<a href="' + linksArray[0] + '" target = "_blank">' + linksArray[0] + '</a>';
			}
		}
		...
		component.set('v.isDiscountApprove',(result.payment_frequency1__c == null) && (result.zqu__PaymentTerm__c == null) && (result.Irregular_Frequency__c == false));
		component.set('v.isNewSubFromExistingAcc',result.Is_from_existing_Billing_Account__c);
		component.set('v.isContractSignedManually',result.Is_Contract_Signed_Manually__c);
		...
		if(result.zqu__Previewed_SubTotal__c !== undefined && result.zqu__Previewed_SubTotal__c !== null){
			component.set('v.isSubTotalCalculated', true);
		} else {
			component.set('v.isSubTotalCalculated', false);
		}

		helper.calculateNewSubQuoteFlowMode(component,result.zqu__SubscriptionType__c,result.document_format__c);
		helper.calculateAmendRenewalFlowMode(component,result.zqu__QuoteBusinessType__c);
		helper.calculateisDeltaZero(component,result.zqu__Previewed_Delta_MRR__c,result.zqu__Previewed_Delta_TCV__c);
