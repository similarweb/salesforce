        ...
        if (responseObj.success) {
          if (responseObj.message == "Add a comment") {
            ...
            helper.showModalPopupComment(
              component,
              "Add comment for approval request"
            );
          } else if (responseObj.message == "Rejected") {
            helper.showModalPopup(
              component,
              "Error",
              "Deal size less than $5000 is not approved according to the company policy.<br/>Please consult with your manager or change the deal size to meet this requirement."
            );
          } else if (responseObj.message == "Pending") {
            var pendingMessage = responseObj.message;
            if (label == "Deal Approval") {
              pendingMessage =
                "The quote was submitted successfully and it's pending GTM approval";
            } else if (label == "Finance Approval") {
              pendingMessage =
                "The quote was submitted successfully and it's pending Finance approval";
            }
