...
export default class AccountConfidenceTable extends LightningElement {
    ...
    handleAccount({error, data}) {
        if (data) {
            ...
            this.swIndustryValue = this.mapToConfidence(data.fields.SW_Industry_Confidence__c.value);
            this.Company_size = this.mapToConfidence(data.fields.Company_Size_Confidence__c.value);
            this.Website_Domain = this.mapToConfidence(data.fields.Website_Domain_Confidence__c.value);
            this.CountryValue = this.mapToConfidence(data.fields.Country_Confidence__c.value);
            this.Parent_company_HQ = this.mapToConfidence(data.fields.Parent_Company_HQ_Confidence__c.value);
            // For tooltip hover fields
            this.swConnectorWebsiteCategory = getFieldValue(data, 'Account.sw_connector_Website_Category__c');
            this.swIndustry = getFieldValue(data, 'Account.SW_Industry__c');
            this.polygraphCompanyEmployeesEstimated = getFieldValue(data, 'Account.Polygraph_Company_Employees_Estimated__c');
            this.polygraphCountry = getFieldValue(data,'Account.Polygraph_Company_Country_Name__c')
            this.swConnectorNumberOfEmployees = getFieldValue(data, 'Account.sw_connector_Number_of_Employees__c');
            ...
            this.websiteClearbit = getFieldValue(data, 'Account.Website_Clearbit__c');
             this.swConnectorSwIdName = getFieldValue(data, 'Account.sw_connector__swId__r.Name');
            this.websiteValue = getFieldValue(data, 'Account.Website');
            this.swTrafficCountry = getFieldValue(data, 'Account.sw_connector_Top_Traffic_Country__c');
            console.log("swTrafficCountry value: ", this.swTrafficCountry);
            this.polygraphCompanyCountryName = getFieldValue(data, 'Account.Polygraph_Company_Country_Name__c');
            this.swConnectorTopTrafficCountry = getFieldValue(data, 'Account.sw_connector_Top_Traffic_Country__c');
            this.billingCountryValue = getFieldValue(data, 'Account.BillingCountry');
            this.swConnectorHeadquartersLocation = getFieldValue(data, 'Account.sw_connector_Headquarters_Location__c');
            this.parentCompanyHQCountry = getFieldValue(data, 'Account.Parent_Company_HQ_Country__c');
            // Map the confidence score to one of 5 values based on your criteria here
            this.confidenceScore = this.mapToConfidence(data.fields.Average_Confidence_Score__c.value);
