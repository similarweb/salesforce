...
const FIELDS = ['Account.Parent_Company_Domain__c','Account.Website_Domain__c'];
...
export default class HierarchyData extends LightningElement {
    ...
    @track gridColumns = [
        ...
        { type: 'text', fieldName: 'Number_of_Open_Opps__c', label: 'Number Of Open opps'},
    ...
    fetchHierarchyData() {
        if (this.parentCompanyDomain || this.websiteDomain) {
            getHierarchyData({ parentCompanyDomain: this.parentCompanyDomain,websiteDomain:this.websiteDomain, accountId: this.recordId })
                .then(result => {
                    this.data = result;
                    ...
                    console.log('Transformed Data:', JSON.stringify(this.gridData, null, 2));
                })
                .catch(error => {
