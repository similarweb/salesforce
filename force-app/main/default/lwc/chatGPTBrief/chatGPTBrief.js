...
export default class ChatGPTBrief extends LightningElement {
    ...
    get fields() {
        ...
        if(this.recordId.startsWith('00Q')) return ['Lead.ChatGPT_Summary__c', 'Lead.Website'];
        if(this.recordId.startsWith('001')) return ['Account.ChatGPT_Summary__c', 'Account.Website'];
        return [];
    }
    ...
    wiredRecord({ error, data }) {
        if (data) {
            this.accountBrief = data.fields ?
                (data.fields.ChatGPT_Summary__c ? data.fields.ChatGPT_Summary__c.value : null)
                : null;
        } else if (error) {
