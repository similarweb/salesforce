...
export default class AccountConfidenceTable extends LightningElement {
    ...
    handleAccount({error, data}) {
        if (data) {
            ...
            this.swIndustryValue = this.mapToConfidence(getFieldValue(data,'Contact.Account.SW_Industry_Confidence__c'));
            this.Company_size = this.mapToConfidence(getFieldValue(data,'Contact.Account.Company_Size_Confidence__c'));
            this.Website_Domain = this.mapToConfidence(getFieldValue(data,'Contact.Account.Website_Domain_Confidence__c'));
            this.CountryValue = this.mapToConfidence(getFieldValue(data,'Contact.Account.Country_Confidence__c'));
            this.Parent_company_HQ = this.mapToConfidence(getFieldValue(data,'Contact.Account.Parent_Company_HQ_Confidence__c'));
            // For tooltip hover fields
            this.swConnectorWebsiteCategory = getFieldValue(data, 'Contact.Account.sw_connector_Website_Category__c');
            this.swIndustry = getFieldValue(data, 'Contact.Account.SW_Industry__c');
            this.polygraphCompanyEmployeesEstimated = getFieldValue(data, 'Contact.Account.Polygraph_Company_Employees_Estimated__c');
            this.polygraphCountry = getFieldValue(data,'Contact.Account.Polygraph_Company_Country_Name__c')
            this.swConnectorNumberOfEmployees = getFieldValue(data, 'Contact.Account.sw_connector_Number_of_Employees__c');
            this.employeesMin = getFieldValue(data, 'Contact.Account.Employees_Min__c');
            this.employeesMax = getFieldValue(data, 'Contact.Account.Employees_Max__c');
            this.websiteClearbit = getFieldValue(data, 'Contact.Account.Website_Clearbit__c');
            this.swConnectorSwIdName = getFieldValue(data, 'Contact.Account.sw_connector__swId__r.Name');
            this.websiteValue = getFieldValue(data, 'Contact.Account.Website');
            this.swTrafficCountry = getFieldValue(data, 'Contact.Account.sw_connector_Top_Traffic_Country__c');
            console.log("swTrafficCountry value: ", this.swTrafficCountry);
            this.polygraphCompanyCountryName = getFieldValue(data, 'Contact.Account.Polygraph_Company_Country_Name__c');
            this.swConnectorTopTrafficCountry = getFieldValue(data, 'Contact.Account.sw_connector_Top_Traffic_Country__c');
            this.billingCountryValue = getFieldValue(data, 'Contact.Account.BillingCountry');
            this.swConnectorHeadquartersLocation = getFieldValue(data, 'Contact.Account.sw_connector_Headquarters_Location__c');
            this.parentCompanyHQCountry = getFieldValue(data, 'Contact.Account.Parent_Company_HQ_Country__c');
            // Map the confidence score to one of 5 values based on your criteria here
            this.confidenceScore = this.mapToConfidence(getFieldValue(data,'Contact.Account.Average_Confidence_Score__c'));
