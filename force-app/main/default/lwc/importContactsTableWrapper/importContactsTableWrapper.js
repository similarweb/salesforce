...
const SUCCESS_MESSAGE = 'Contact import has been started! Please, expect email once it is done.';
const SUCCESS_VARIANT = 'success';
const SUCCESS_DELETE_MESSAGE = 'Drafts are successfully deleted!';
const WARNING_VARIANT = 'warning';
const WARNING_MESSAGE = 'Contact import has been started but it looks like something wrong with some of contacts! Please check it!'
const WARNING_TITLE = 'Warning';
const BASE_VARIANT = 'base';
const BASE_MESSAGE = 'Contact import has been started! Please, expect email once it is done.';
...
export default class ImportContactsTableWrapper extends LightningElement {
    ...
    startTimerLoad(){
        this.isLoaded = false;
        console.log("ImportContactsTableWrapper -> startTimerLoad -> this.isLoaded", this.isLoaded)
    }

    stopTimerLoad(){
        this.isLoaded = true;
        console.log("ImportContactsTableWrapper -> stopTimerLoad -> this.isLoaded", this.isLoaded)
    }

    renderedCallback(){
        console.log('Render check', this._currentPageItems, ' Pages ', JSON.parse(JSON.stringify(this.pages)));
    }
    ...
    handlePageSort(items){
        ...
        this.currentPageItems = JSON.parse(JSON.stringify(pages.find(elem => elem.pageNumber == this.currentPageNumber).items));
        console.log('Pages ', JSON.parse(JSON.stringify(pages)));
        console.log('Number of pages ', JSON.parse(JSON.stringify(this.numberOfPages)));
        return pages;
    }

    handlePageChange(event){
        this.value = event.target.value;
        this.currentPageItems = this.pages.find(elem => elem.pageNumber == this.value).items;
        this.currentPageNumber = this.value;
        console.log('Current page ', this.currentPageNumber, 'Current page items ', JSON.parse(JSON.stringify(this.currentPageItems)));
    }
    ...
    onImportContactsClick(){
        ...
        if(this.pages[this.currentPageNumber].pageNumber == this.currentPageNumber && err){
            this.isLoaded = false;
        ...
        if(err && validTableItems.length == 0){
            ...
            console.log('validTableItems ', JSON.parse(JSON.stringify(validTableItems)));
            console.log('JSON ',JSON.stringify(validTableItems), 'JSON Length', JSON.stringify(validTableItems).length)
            if(validTableItems.length == 0 && invalidTableItems.length == 0){
                ...
                importContacts({contactsToImportJSON : JSON.stringify(validTableItems)})
                .then(result => {
    ...
    saveDraftOnRefresh(event){
        ...
        let index = this.pages[eventObj.pageNumber].items.map(function(e) { return e.draftId; }).indexOf(eventObj.draftId);
        if(index != -1){
            this.pages[eventObj.pageNumber].items.splice(index,1,eventObj.fields);
            this.tableItems.splice(index,1,eventObj.fields);
            console.log('pages after refresh', JSON.parse(JSON.stringify(this.pages)));
        } else {
            this.pages[eventObj.pageNumber].items.splice(eventObj.index,1,eventObj.fields);
    ...
    saveDraft(event){
        let eventObj = JSON.parse(JSON.stringify(event.detail));
        let index = this.pages[this.currentPageNumber].items.map(function(e) { return e.draftId; }).indexOf(eventObj.draftId);
        if(index != -1){
            this.pages[this.currentPageNumber].items.splice(index,1,eventObj.fields);
            this.tableItems.splice(index,1,eventObj.fields);this.pages = [...JSON.parse(JSON.stringify(this.pages))];
            console.log('pages ', JSON.parse(JSON.stringify(this.pages)));
        } else {
            this.pages[this.currentPageNumber].items[eventObj.index] = JSON.parse(JSON.stringify(eventObj.fields));
            this.tableItems.splice(eventObj.index,1,eventObj.fields);

            this.currentPageItems.splice(eventObj.index,1,eventObj.fields);this.pages = [...JSON.parse(JSON.stringify(this.pages))];
            console.log('pages ', JSON.parse(JSON.stringify(this.pages)));
        }
    }
    ...
    importDraftsFromCSV(event){
        ...
        const event1 = new ShowToastEvent({
            title: 'Success',
            message: event.detail.message ? event.detail.message : 'Contacts are successfully imported from CSV',
            variant: 'success',
            mode: 'dismissable'
        });
    ...
    deleteAllDrafts(){
        ...
        for(let j=0; j<this.pages.length; j++){
            for(let i=0;i<this.pages[j].items.length; i++){
                console.log('pages items[i] ', JSON.parse(JSON.stringify(this.pages[j].items[i])));
                if(this.pages[j].items[i].draftId == ''){  
                    console.log('This item is empty');
                } else {
            ...
            deleteDrafts({contactsToImportJSON : JSON.stringify(draftsToDelete)})
                .then(result => {
                    ...
                    if(result.content.length > 0){
                        ...
                        this.dispatchEvent(event);
                        this.tableItems = JSON.parse(JSON.stringify(result.content));
                        this.pages = this.handlePageSort(JSON.parse(JSON.stringify(this.tableItems)));
                    }
                })
                .catch(error => {
