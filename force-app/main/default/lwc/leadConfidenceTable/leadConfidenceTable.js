...
export default class AccountConfidenceTable extends LightningElement {
    ...
    handleAccount({error, data}) {
        if (data) {
            ...
            this.swIndustryValue = this.mapToConfidence(getFieldValue(data,'lead.SW_Industry_Confidence__c'));
            this.Company_size = this.mapToConfidence(getFieldValue(data,'lead.Company_Size_Confidence__c'));
            this.Website_Domain = this.mapToConfidence(getFieldValue(data,'lead.Website_Domain_Confidence__c'));
            this.CountryValue = this.mapToConfidence(getFieldValue(data,'lead.Country_Confidence__c'));
            ...
            this.swConnectorWebsiteCategory = getFieldValue(data, 'lead.sw_connector__swId__r.sw_connector__Website_Category__c');
            this.swIndustry = getFieldValue(data, 'lead.SW_Industry__c');
            this.polygraphCompanyEmployeesEstimated = getFieldValue(data, 'lead.Polygraph_Company_Employees_Estimated__c');
            this.polygraphCountry = getFieldValue(data,'lead.Polygraph_Company_Country_Name__c')
            this.swConnectorNumberOfEmployees = getFieldValue(data, 'Lead.sw_connector__swId__r.sw_connector__Employee_Range__c');
            ...
            this.swConnectorSwIdName = getFieldValue(data, 'lead.sw_connector__swId__r.Name');
            this.websiteValue = getFieldValue(data, 'lead.Website');
            this.swTrafficCountry = getFieldValue(data, 'lead.sw_connector__swId__r.sw_connector__Top_Traffic_Country__c');
            console.log("swTrafficCountry value: ", this.swTrafficCountry);
            this.polygraphCompanyCountryName = getFieldValue(data, 'lead.Polygraph_Company_Country_Name__c');
            this.swConnectorTopTrafficCountry = getFieldValue(data, 'lead.sw_connector__swId__r.sw_connector__Top_Traffic_Country__c');
            ...
            this.parentCompanyHQCountry = getFieldValue(data, 'lead.Parent_Company_HQ_Country__c');
            // Map the confidence score to one of 5 values based on your criteria here
            this.confidenceScore = this.mapToConfidence(getFieldValue(data,'lead.Average_Confidence_Score__c'));
