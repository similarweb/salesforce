...
export default class MqlScoreLwc extends LightningElement {
    ...
    async loadRecordData() {
        try {
            const data = await getRecordData({ recordId: this.recordId, sObjectType:this.sObjectType});
    ...
    get indicatorStyle() {
        ...
        if (this.score) {
            ...
            const activeSection = this.template.querySelector(`.${activeSectionClass}`);
    ...
    convertToNicerVersion(sfdcDatetime) {
        ...
        if (sfdcDatetime) {
            ...
            if (date.toDateString() === now.toDateString()) {
                ...
                const fullDateOptions = { year: 'numeric', month: 'long', day: 'numeric', ...options };
