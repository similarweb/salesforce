...
export default class ProductTracking extends LightningElement {
    upsellSubTypeOptions = [
        {label: 'Upsell (New Paying Account)', value: 'Upsell (New Paying Account)'},
    ...
    connectedCallback() {
        getData({recordId: this.recordId})
            .then((data) => {
                ...
                this.checkSubTypeAvailability = this.type !== 'New Sale' && this.type !== 'Renewal' && this.type !== 'Upsell';

                if (this.type === 'New Sale' || this.type === 'Renewal') {
                    this.subType = this.type;
                } else if (this.type === 'Upsell') {
                    this.subType = this.subType ? this.subType : (this.isAccActive ? 'Upsell (Existing Account)' : 'Upsell (New Paying Account)');
    ...
    handlePurchaseMotionChange(event) {
        const solutionIndex = event.target.dataset.id, skuIndex = event.target.dataset.index, value = event.target.value;

        this.solutions[solutionIndex].products[this.subType][skuIndex].purchaseMotion = value;
        this.solutions[solutionIndex].skus[skuIndex].purchaseMotion = value;
    }

    handleProductSelect(event) {
        const solutionIndex = event.target.dataset.id, skuIndex = event.target.dataset.index, checked = event.target.checked;

        this.solutions[solutionIndex].products[this.subType][skuIndex].selected = checked;
        this.solutions[solutionIndex].skus[skuIndex].selected = checked;

        if (((checked === true && this.hasAtLeast1SelectedProduct === false) ||
    ...
    handleToggle(event) {
        const index = event.currentTarget.dataset.index, checked = event.currentTarget.checked;
    ...
    validateBody() {
        ...
        let emptySolutions = [], skusWithoutPurchaseMotion = [], noPriceProducts = [];
        this.solutions.forEach(s => {
            if (s.open === true) {
                let hasAtLeast1SelectedProduct = false;
                s.skus.forEach(sku => {
        ...
        if (emptySolutions.length > 0) {
            this.dispatchEvent(new ShowToastEvent({
                title: 'Warning',
                variant: 'warning',
                message: 'Select at least one add-on / package for solutions: ' + emptySolutions.join(', ')
            }));
            return 'Failure';
        }
        ...
        if (noPriceProducts.length > 0) {
            this.dispatchEvent(new ShowToastEvent({
                title: 'Warning',
                variant: 'warning',
                message: 'Following selected products don\'t have a pricebook entry: ' + noPriceProducts.join(', ') + '. Please deselect them or add to the pricebook'
            }));
            return 'Failure';
        }
    ...
    filterSolutions() {
        let solutions = this.subType ? this.allSolutions.filter(s => s.availableSubTypes.includes(this.subType)) : [];

        solutions.forEach(s => {
            s.skus = this.subType ? s.products[this.subType] : [];

            s.skus.forEach(sku => {
                if (this.acquiredCompany || this.subType === 'New Sale' || this.subType === 'Upsell (New Paying Account)') {
                ...
                if (this.subType === 'Upsell (Existing Account)' && !this.acquiredCompany) {
                    sku.purchaseMotionDisabled = sku.purchaseMotion != undefined;
                    sku.purchaseMotionOptions = sku.purchaseMotionOptions.length > 0 ? sku.purchaseMotionOptions : this.allPurchaseMotionOptions;
                }
            });
        })
