...
export default class RenewalAiAwaiting extends LightningElement {
  ...
  connectedCallback() {
    ...
    this.maxPollCount = Math.floor(
      this.maxWaitTimeSeconds / this.pollIntervalSeconds
    );

    if (this.fieldsToCheck) {
      this.fieldApiNames = this.fieldsToCheck
        .split(",")
  ...
  handleTimeout() {
    this.isLoading = false;
    this.displayMessage =
      "Processing is taking longer than expected. Continuing...";
