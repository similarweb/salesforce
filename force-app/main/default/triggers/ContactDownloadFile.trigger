    ...
    for(Contact con:Trigger.new){
        ...
        if(Trigger.isUpdate){
            ...
            if(con.Form_Submission_Date_and_Time__c!=null&&con.Form_Submission_Date_and_Time__c!=Trigger.OldMap.get(con.id).Form_Submission_Date_and_Time__c){
                ...
                if(formname.contains('Webinar Recorded')||formname.contains('Report')) objs.add(new ZCampaignAutomation.objcheck('Contact', con.id, formname, subj));    
            }
            if((con.Web_Form_Name__c!=Trigger.oldMap.get(con.id).Web_Form_Name__c||con.Webinar_Name__c!=Trigger.oldMap.get(con.id).Webinar_Name__c)&&formname.contains('Zoom')&&(formname.contains('Registered')||formname.contains('Attended'))){
                String subj = con.Webinar_Name__c!=null?con.Webinar_Name__c:'';
                objs.add(new ZCampaignAutomation.objcheck('Contact', con.id, formname, subj));
            }
        }
            ...
            if(formname.contains('Zoom')&&(formname.contains('Registered')||formname.contains('Attended'))){
                String subj = con.Webinar_Name__c!=null?con.Webinar_Name__c:'';
                objs.add(new ZCampaignAutomation.objcheck('Contact', con.id, formname, subj));
            }
        }
