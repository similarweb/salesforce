trigger OpportunityTrigger on Opportunity(
  ...
  if (Trigger.isBefore) {
    if (Trigger.isInsert) {
      ...
      OpportunityTriggerHandler.setForecastCategoryName(
    ...
    if (Trigger.isUpdate) {
      OpportunityTriggerHandler.enrichContactDataByPolygraph(
        Trigger.new,
        Trigger.oldMap
      );
      ...
      OpportunityTriggerHandler.clearClosedLostValues(
        (List<Opportunity>) Trigger.new,
        (Map<Id, Opportunity>) Trigger.oldMap
      );
      OpportunityTriggerHandler.changeCloseDateToToday(
        (List<Opportunity>) Trigger.new,
        (Map<Id, Opportunity>) Trigger.oldMap
      );
      OpportunityTriggerHandler.setForecastCategoryName(
  ...
  if (Trigger.isAfter) {
    ...
    if (Trigger.isInsert) {
      ...
      OpportunityTriggerHandler.manageOpportunityForCommunityUsers(
        (List<Opportunity>) Trigger.new,
        null
      );
