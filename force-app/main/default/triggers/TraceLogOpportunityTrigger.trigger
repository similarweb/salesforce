    ...
    for (Opportunity newOpp : (List<Opportunity>)Trigger.new) {
        try {
            ...
            if (newOpp.Name != Trigger.oldMap.get(newOpp.Id).get('Name')) {
                ...
                traceLog.Log__c = System.Request.getCurrent().getQuiddity()+'  | *********** |  ';
                traceLog.Log__c += ' Username : ' + UserInfo.getUserName()+'  | *********** |  ';
                ...
                for (String fieldName : fieldMap.keySet()) {
                    ...
                    if (newVal != oldVal) {
                        traceLog.Log__c += fieldName + ' : ' + (newVal != null ? String.valueOf(newVal) : 'null')+ ' ->->-> ' + (oldVal != null ? String.valueOf(oldVal) : 'null') + '  |---------------------------------------------------|  ';
                    }
                }
